...
class ColumnChunkPageReadStore implements PageReadStore, DictionaryPageReadStore {
   ...
   */
  static class ColumnChunkPageReader implements PageReader {

    protected final BytesDecompressor decompressor;
    protected final long valueCount;
    protected final List<DataPage> compressedPages;
    protected final DictionaryPage compressedDictionaryPage;
    ...
    static class ColumnChunkPageReaderV2 extends ColumnChunkPageReader {

      ColumnChunkPageReaderV2(BytesDecompressor decompressor, List<DataPage> compressedPages, DictionaryPage compressedDictionaryPage) {
        super(decompressor, compressedPages, compressedDictionaryPage);
      }
      ...
      public DataPage readPage() {
        ...
        return compressedPage.accept(new DataPage.Visitor<DataPage>() {
          ...
          public DataPage visit(DataPageV1 dataPageV1) {
            try {
              return new OapParquetDataPageV1(
                decompressor.decompress(dataPageV1.getBytes(), dataPageV1.getUncompressedSize()),
                dataPageV1.getValueCount(),
                dataPageV1.getUncompressedSize(),
                dataPageV1.getStatistics(),
                ...
                dataPageV1.getDlEncoding(),
                dataPageV1.getValueEncoding(),
                ((OapParquetDataPageV1)dataPageV1).getPageRowStartNum());
            } catch (IOException e) {
              throw new ParquetDecodingException("could not decompress page", e);
            }
          }
            ...
            try {
              int uncompressedSize = Ints.checkedCast(
                dataPageV2.getUncompressedSize()
                  - dataPageV2.getDefinitionLevels().size()
                  - dataPageV2.getRepetitionLevels().size());
              return OapParquetDataPageV2.uncompressed(
                dataPageV2.getRowCount(),
                dataPageV2.getNullCount(),
                dataPageV2.getValueCount(),
                dataPageV2.getRepetitionLevels(),
                ...
                decompressor.decompress(dataPageV2.getData(), uncompressedSize),
                dataPageV2.getStatistics(),
                ((OapParquetDataPageV2)dataPageV2).getPageRowStartNum()
              );
            } catch (IOException e) {
              throw new ParquetDecodingException("could not decompress page", e);
            }
