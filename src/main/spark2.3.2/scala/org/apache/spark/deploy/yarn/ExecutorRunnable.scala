...
private[yarn] class ExecutorRunnable(
  ...
  private def prepareCommand(): List[String] = {
    ...
    */
    ...
    val isNumaBindNeeded = numaManager.isNumaBindEnable()
    val numaId = numaManager.calAndGetExecutorNumaId(executorId, hostname)

    val numaCtlCommand = if (isNumaBindNeeded && executorId != "<executorId>") {
      numaId match {
        case Some(id) =>
          val command = s"numactl --cpubind=${id} --membind=${id}"
          command
        case None =>
          throw new UnsupportedOperationException(
            s"can't find needed numa id for executor ${executorId}")
      }
    }

    YarnSparkHadoopUtil.addOutOfMemoryErrorArgument(javaOpts)
    val commands = if (isNumaBindNeeded) {
      prefixEnv ++
